...
class TrainFlowPolicyWorkspace:
    ...
    def run(self):
        ...
        # Hydra changes cwd to the run output dir; keep data paths stable.
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            # run rollout (skip during lightweight success-rate check epochs)
            run_full_rollout = (
                (self.epoch % cfg.training.rollout_every) == 0
                and RUN_ROLLOUT
                and env_runner is not None
            )
            if use_early_stopping and early_stop_manager is not None:
                if early_stop_manager.should_run_success_check(self.epoch):
            ...
            if (
                use_early_stopping
                and early_stop_manager is not None
                and early_stop_manager.should_run_success_check(self.epoch)
                and env_runner is not None
            ):
                es_cfg = early_stop_manager.config
                light_log = env_runner.run(
                ...
                with torch.no_grad():
                    ...
                    if len(val_losses) > 0:
                        ...
                        if early_stop_manager is not None:
                            improved = early_stop_manager.update_val_loss(
                                self.epoch, val_loss
                            )
            ...
            if (
                use_early_stopping
                and early_stop_manager is not None
                and early_stop_manager.check_stop(self.epoch)
            ):
                cprint(
        ...
        if use_early_stopping and early_stop_manager is not None:
            ...
            st = early_stop_manager.state
            summary = {
                "stopped_epoch": self.epoch,
                "early_stopped": bool(st.should_stop),
            ...
            summary_path = pathlib.Path(self.output_dir) / "training_summary.json"
            with open(summary_path, "w") as f:
                json.dump(summary, f, indent=2)
            cprint(f"Wrote training summary to {summary_path}", "cyan")
    ...
    def eval(self):
        ...
        ckpt_path = self._get_best_val_checkpoint_path()
        if ckpt_path is None:
            ckpt_path = self.get_checkpoint_path(tag="latest")
        if ckpt_path.is_file():
            cprint(f"Eval from checkpoint {ckpt_path}", "magenta")
