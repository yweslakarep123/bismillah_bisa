...
from flow_policy_3d.model.vision.state_encoder import StateOnlyEncoder
import warnings
...
class FlowPolicy(BasePolicy):
    def __init__(self, 
        ...
        self.state_only = obs_encoder_type == "state_only"
        ...
        if self.state_only:
            ...
            cprint("[FlowPolicy] obs_encoder_type: state_only", "yellow")
        else:
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if not self.state_only and not self.use_pc_color:
            nobs['point_cloud'] = nobs['point_cloud'][..., :3]
        value = nobs['agent_pos'] if self.state_only else next(iter(nobs.values()))
        B, To = value.shape[:2]
        T = self.horizon
        Da = self.action_dim
